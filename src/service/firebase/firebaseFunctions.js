  ...
  try {
    ...
    while (idQuerySnapshot.size > 0) {
      ...
      idQuerySnapshot = await getDocs(
        query(farmerRef, where("farmer_id", "==", farmer_id))
      );
    }
...
export const addMessageData = async (
  ...
  try {
    ...
    // Set the document with the specified ID
    const farmerDocRef = doc(db, "Alert", alert_id);
...
//add farmer
let sL_ID = 101; // Initialize the ID counter

export const addSoilData = async (
  sd_id,
  nitrogen,
  phosphorus,
  potassium,
